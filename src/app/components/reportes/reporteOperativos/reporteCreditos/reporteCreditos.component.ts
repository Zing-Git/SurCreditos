...
export class ReporteCreditos implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.creditoService.postGetAllCreditosTodosLosUsuarios(this.session).subscribe(result => {
      ...
      /*this.creditos.forEach(x =>{
         if (x.planPagos.finalizado) {
          this.planesFinalizados  += 1;
        } else {
          this.planesActivos += 1;
        }
      })*/
    })

    this.creditoService.postGetDineroCirculante(this.session.token).subscribe(result => {
      this.dineroCirculante = result;
      // console.log(result);
    })
  ...
  getData() {
    ...
    let parameter = {
      ...
      diaInicio: this.utilidades.getDayString(fechaInicio.getDate() + 1),
      diaFin: this.utilidades.getDayString(fechaFin.getDate() + 1),
      mesInicio: fechaInicio.getMonth() + 1,
      mesFin: fechaFin.getMonth() + 1,
      anioInicio: fechaInicio.getFullYear(),
      anioFin: fechaFin.getFullYear()
    }
    let fechaI = new Date(parameter.anioInicio + '/' + parameter.mesInicio + '/' + parameter.diaInicio);
    let fechaF = new Date(parameter.anioFin + '/' + parameter.mesFin + '/' + parameter.diaFin);
   /*  console.log(fechaI);
    console.log(fechaF); */
    if (fechaI.getFullYear() > 1971 && fechaF.getFullYear() > 1971) {
      ...
      for (var i = 0; i < this.creditos.length; i++) {
        ...
        if (fecha.valueOf() >= fechaI.valueOf() && fecha.valueOf() <= fechaF.valueOf()) {
